    ...
    getVideos: (data) => {
        return new Promise((resolve, reject) => {
            if (!data || ((!data.id || data.id.length === 0) && (!data.user_id || data.user_id.length === 0)) && (!data.game_id || data.game_id.length === 0)) {
                ...
                queries = queryString.stringify({ id: data.id, user_id: data.user_id, game_id: data.game_id, first: data.first, language: data.language, period: data.period, sort: data.sort, type: data.type, after: data.after, before: data.before });
                request.get(`${index.apiUrl}/videos?` + queries, {}).then(function(data) {
