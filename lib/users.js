...
module.exports = {
    getUsers: (data) => {
        return new Promise((resolve, reject) => {
            if (!data || ((!data.id || data.id.length === 0) && (!data.login || data.login.length === 0))) {
                ...
                request.get(`${index.apiUrl}/users?` + queries, {}).then(function(data) {
    ...
    getUsersFollows: (data) => {
        return new Promise((resolve, reject) => {
            if (!data || (!data.from_id && !data.to_id)) {
                ...
                request.get(`${index.apiUrl}/users/follows?` + queries, {}).then(function(data) {
                    resolve(helpers.generatePayload(200, "success", "OK", data));
                });
            }
        });
    ...
    getUserTags: (data) => {
        return new Promise((resolve, reject) => {
            if (!data || !data.id) {
                resolve(helpers.generatePayload(400, "bad_request", "You must specify a user ID.", null));
            }
            else if (typeof data.id !== 'string' && data.id.length > 1) {
                resolve(helpers.generatePayload(400, "bad_request", "You have specified too many user IDs. The maximum is 1.", null));
            }
            else {
                queries = queryString.stringify({ broadcaster_id: data.id });
                request.get(`${index.apiUrl}/streams/tags?` + queries, {}).then(function(data) {
                    resolve(helpers.generatePayload(200, "success", "OK", data));
                });
            }
        });
    ...
    updateUser: (data) => {
        return new Promise((resolve, reject) => {
            if (!data || !data.token) {
                ...
                request.put(`${index.apiUrl}/users?` + queries, { token: data.token }).then(function(data) {
                    resolve(helpers.generatePayload(200, "success", "OK", data));
                });
            }
        });
